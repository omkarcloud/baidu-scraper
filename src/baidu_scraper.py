...
def print_data_errors(credits_exhausted, not_subscribed, unknown_error, no_key):
    ...
    if credits_exhausted:
        ...
        print(f"Could not get data for {len(credits_exhausted)} {name} due to credit exhaustion. Please consider upgrading your plan by visiting https://rapidapi.com/Chetan11dev/api/baidu-scraper/pricing to continue scraping data.")
...
class Baidu:
    ...
    def search(query:  Union[str, List[str]],  max: Optional[int] = None, key: Optional[str] =None,  use_cache: bool = True) -> Dict:
        """
        Function to scrape data from Baidu.
